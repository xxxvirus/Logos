...
public interface ShopDao extends JpaRepository<Shop, Integer> {
	...
	@Query("select s from Shop s where s.shyear=?1 and s.shprice=?2 and s.isbn=?3 and s.edition=?4 and s.page=?5 and s.titleSh.id=?6 and s.shSeria.id=?7 and s.shlang.id=?8 and s.shcat.id=?9")
	Shop findUnique(int shyear, BigDecimal shprice, String isbn, int edition,
			int page, int titleShopId, int seriaPubId, int langId,
			int categoryId);
	@Query("select s from Shop s where s.titleSh.book.author.id=?1")
	List<Shop> findByAuthorId(int id);
