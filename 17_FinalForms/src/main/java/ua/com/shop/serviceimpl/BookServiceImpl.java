...
//	}
	...
	public void addGenre(int id, int genreId) {
		Book book = bookDao.loadedGenre(id);
	...
	@Override
	public Book loadgenre(int id) {
		return bookDao.loadedGenre(id);
	}

	@Override
	public void saveL(Book book, List<Integer> collect) {
		List<Genre> list = genreDao.findAll(collect);
		book.setGenre(list);
		bookDao.save(book);
	}
