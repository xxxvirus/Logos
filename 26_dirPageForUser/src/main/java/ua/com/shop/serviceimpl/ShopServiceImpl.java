...
//		return shopDao.findByAuthorId(id, new ShopSpecification(filter), pageable);
//	}
	...
	@Override
	public List<ShopDto> findByAuthorId(int id) {
		return shopDao.findByAuthorId(id).stream().map(this::map).collect(Collectors.toList());
	}
	
	public List<ShopDto> findAllDto() {
		return shopDao.findAll().stream().map(this::map).collect(Collectors.toList());
	}
	
	private ShopDto map(Shop shop){
		ShopDto dto = new ShopDto();
		dto.setId(shop.getId());
		dto.setAnotation(shop.getAnotation());
		dto.setEdition(shop.getEdition());
		dto.setIsbn(shop.getIsbn());
		dto.setShcatName(shop.getShcat().getNameOfC());
		dto.setShlangName(shop.getShlang().getLang());
		dto.setShpage(shop.getShpage());
		dto.setShprice(shop.getShprice());
		dto.setShSeriaName(shop.getShSeria().getNameOfS());
		dto.setShyear(shop.getShyear());
		dto.setTitleShName(shop.getTitleSh().getNamePub());
		return dto;
	}
